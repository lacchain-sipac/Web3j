...
public class ECDSAAlgorithm {
	...
	public ECDSAAlgorithm(String algorithm, int ecNumberSize) throws IllegalArgumentException {
		this.algorithm = algorithm;
		this.ecNumberSize = ecNumberSize;
	}

	public boolean verify(PublicKey publicKey, String header, String body, String signer) throws Exception {

		if (publicKey == null) {
			throw new IllegalStateException("The given Public Key is null.");
		}

		return new CryptoHelper().verifySignatureFor(algorithm, publicKey, header, body,
				JOSEToDER(Base64.decodeBase64(signer), ecNumberSize));

	}

	public byte[] sign(PrivateKey privateKey, byte[] headerBytes, byte[] payloadBytes) throws Exception {
		if (privateKey == null) {
			throw new IllegalStateException("The given Private Key is null.");
		}
		byte[] signature = new CryptoHelper().createSignatureFor(algorithm, privateKey, headerBytes, payloadBytes);

		return DERToJOSE(signature, ecNumberSize);
